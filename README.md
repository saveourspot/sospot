...
경계값은 해당 등급에 포함합니다. 업종별 이상징후 점수는 `중점검토 80 이상`, `주의 65 이상 80 미만`, `관심 50 이상 65 미만`, `정상 50 미만`입니다. 지도에 표시하는 행정동 종합 등급은 82개 동의 상대 순위인 백분위를 기준으로 `중점검토 90 이상`, `주의 70 이상 90 미만`, `관심 40 이상 70 미만`, `정상 40 미만`입니다.
...
| 소상공인 경기동향(BSI) 현황 | 대전 전체 경기 흐름을 설명하는 보조 맥락 (이상징후 점수 계산에는 미사용) |
